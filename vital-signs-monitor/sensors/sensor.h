 ...
 * smart ICU device and functions to generate new samples inside given ranges.
 * A generic sensor implementation must define a process simulating the sampling activity;
 * in particular, this process must:
 ...
 *    and posted to the subscriber.
 * 2) stop generating samples when it receives a stop event.
...
/**
 * \brief                   Generate a new integer sample in a given range.
 ...
 * \param lower_bound       The lower bound (inclusive) of the sample range.
 * \param upper_bound       The upper bound (inclusive) of the sample range.
 * \return                  A new integer sample.
 *
 *                          Generate a new integer sample in a given range.
 *                          A new sample is derived from the last one by adding or subtracting a deviation,
 *                          respecting the upper and lower bounds of the range.
 *                          The deviation is a random number in the range [-max_deviation, max_deviation].
 */
